...
double growth_factor_anyz(double z)
{
 ...
 return 1.0 / (1.0 + z) * 2.5 * omega_m_z / (pow(omega_m_z, 4.0/7.0) 
        - omega_v_z + (1.0 + omega_m_z * 0.5) * (1.0 + omega_v_z / 70.0));
...
double growth_factor(double z)   /* normalized growth factor */     
{
  return growth_factor_anyz(z) / growth_factor_anyz(z_i);	
}	                             /* end growth_factor */
...
double f_k(double k, double R)              /* defined as T_k^2  * k^(2+ns) * W(k*R)^2 */
{
 double tk = T_k(k);
...
double un_norm_sigma_square(double z, double R)
{
 ...
 return result;
}                                   /* end un_norm_sigma_square */

void init_power_norm()
{		
  norm_pow_A = Sigma_8 * Sigma_8 / un_norm_sigma_square(0, 8);	
  printf("norm_pow_A = %.8e\n", norm_pow_A);
}                                   /* end init_power_norm */
...
double sigma_square(double z, double M)
{
  ...
  return norm_pow_A * un_norm_sigma_square(z, R);	
